...
BOOL CALLBACK DlgProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_NOTIFY:
	{
		//Здесь лучше отслеживать изменения префикса, а изменения маски лучше отслеживать в сообщении 'WM_COMMAND',
...
VOID PrintInfo(HWND hwnd)
{
	...
	sprintf(sz_NetworkIP_buffer, "Адрес сети: \t\t\t%s", IPaddressToString(dwIPaddress, sz_buffer));
